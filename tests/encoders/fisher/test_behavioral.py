"""Behavioural / requirement-driven tests for :class:`pyvisim.encoders.FisherVectorEmbedder`."""
...
from pyvisim.encoders import FisherVectorEmbedder
...
def test_same_category_scores_higher_than_different(
    ...
    _assert_same_category_higher(learned_fisher_embedder, category_query_images)


def test_identical_image_near_perfect_similarity(
    learned_fisher_embedder: FisherVectorEmbedder, checkerboard_image: ImageObj
) -> None:
    """Fisher reports similarity of 1.0 for an image against its copy."""
    image = checkerboard_image.array
    score = float(
        learned_fisher_embedder.similarity_score([image], [image.copy()])[0, 0]
    )
...
def test_embed_deterministic(
    ...
    image = checkerboard_image.array
    assert np.allclose(
        learned_fisher_embedder.embed([image]), learned_fisher_embedder.embed([image])
    )
...
def test_same_noise_more_similar_than_different_noise(
    ...
    _assert_same_noise_higher(
        learned_fisher_embedder, noisy_image_pair, very_noisy_image_pair
    )
...
def test_different_noise_more_similar_than_grey(
    ...
    _assert_grey_is_floor(learned_fisher_embedder, noisy_a.array, solid_image.array)
...
def test_full_noise_ordering(
    ...
    _assert_grey_is_floor(
        learned_fisher_embedder, noisy_image_pair[0].array, solid_image.array
    )
...
def test_embed_invariant_after_serialization(
    ...
    """Fisher embeds an image to the exact same vector before and after a save/load round-trip."""
    image = checkerboard_image.array
    vector_before = learned_fisher_embedder.embed([image])

    path = learned_fisher_embedder.save_to_disk(tmp_path / "embedder")
    try:
        loaded = FisherVectorEmbedder.load_from_disk(path)
