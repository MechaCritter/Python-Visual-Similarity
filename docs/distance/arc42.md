...
argument. This is why the accepted names (`"cosine"`, `"euclidean"`, `"l1"`,
...
either input, so its extra memory is the result matrix no matter how large the
inputs are. `manhattan_distances` cannot avoid a broadcast temporary the same
way, so it caps the size of that temporary instead, through the
