...
custom descriptor into the embedders without writing a new `FeatureExtractorBase`
subclass.
...
- `output_dim` is supplied explicitly because, unlike SIFT or a CNN layer, an arbitrary
  function has no inspectable descriptor size. The embedders need this value to validate
