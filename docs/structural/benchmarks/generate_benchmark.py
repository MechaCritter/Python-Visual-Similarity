...
Outputs, written next to this script (``docs/structural/benchmarks/``) by
default:

- ``benchmark_results.json`` — per-image errors (with the baseline spelled
  out), raw runtimes and the full configuration.
- ``{ssim,ms_ssim}_runtime_barplot.png`` — median pyvisim vs. baseline
  runtimes.
- The auto-generated ``## Benchmark`` section of ``docs/structural/README.md``,
  replaced in-place between the ``benchmark:begin`` / ``benchmark:end``
  markers.

Usage (from the repository root)::

    python docs/structural/benchmarks/generate_benchmark.py [--seed N] [--repeats N]
"""
...
> [`{_SCRIPT_REF}`](benchmarks/generate_benchmark.py) (see
> [`benchmarks/benchmark_results.json`](benchmarks/benchmark_results.json) for
> the raw data); re-run
> `python {_SCRIPT_REF}` to refresh it.
