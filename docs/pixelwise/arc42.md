...
which owns the shared pipeline around the metric itself (input normalization,
shape validation and memory-bounded pair batching).
...
Cython source. This is also why every compared pair must have the same
