...
* `BCESiameseNetwork` **learns the comparison itself**: a scoring layer on top of the
...
* A **calibrated "same class?" probability** for pairs, e.g. verification or one-shot
  recognition → `BCESiameseNetwork`. The network itself scores the pair.
...
### `BCESiameseNetwork` (Koch et al., 2015)
...
classifier = BCESiameseNetwork(backbone="resnet18", embedding_dim=128, device="cuda")
...
forward pass and returns one L2-normalized embedding per image:
...
**BCE variant.** The network is a binary classifier over pairs: `forward` takes both
