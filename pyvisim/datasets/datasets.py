...
def _download_and_process_file(url: str, dest: str, extract_dir: str) -> None:
    ...
    if dest.endswith(".zip"):
        _extract_zip(
            dest, os.path.join(extract_dir, os.path.splitext(os.path.basename(dest))[0])
        )
        os.remove(dest)
...
def _extract_zip(zip_file: str, extract_to: str) -> None:
    ...
    with zipfile.ZipFile(zip_file, "r") as zf:
        ...
        with tqdm(
            total=total_files,
            unit="file",
            desc=f"Extracting {os.path.basename(zip_file)}",
        ) as progress_bar:
...
def _extract_tar(tar_file: str, extract_to: str) -> None:
    ...
    with tarfile.open(tar_file, "r:gz") as tf:
        ...
        with tqdm(
            total=total_files,
            unit="file",
            desc=f"Extracting {os.path.basename(tar_file)}",
        ) as progress_bar:
...
def _check_data_integrity() -> bool:
    ...
    if not os.path.isfile(_IMAGE_LABEL_FILE):
        ...
        return False
    try:
        mat_data = loadmat(_IMAGE_LABEL_FILE)
    ...
    try:
        ...
        if (
            len(tstid) != NUM_TEST_IMG
            or len(valid) != NUM_VAL_IMG
            or len(trnid) != NUM_TRAIN_IMG
        ):
            logger.warning(
                f"setid.mat has incorrect lengths. tstid={len(tstid)}, "
...
class OxfordFlowerDataset(Dataset[tuple[UInt8NumpyArray, int, str]]):
    ...
    def __init__(self, purpose: str | list[str] = "train") -> None:
        ...
        if len(set(self.purpose)) < len(self.purpose):
            raise ValueError(
                "Duplicate purposes found in the list. Please provide unique purposes."
            )
    ...
    def _load_image_paths(self) -> list[str]:
        ...
        """
        images = sorted([f for f in os.listdir(_IMAGE_DIR) if f.endswith(".jpg")])
    ...
    def _filter_by_purpose(self) -> tuple[list[str], list[int]]:
        ...
        for p in self.purpose:
            match p:
                ...
                case _:
                    raise ValueError(
                        f"Unknown purpose: {p}. Must be 'train', 'validation', or 'test'."
                    )
