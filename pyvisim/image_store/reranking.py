...
class KReciprocalReranker:
    ...
    neighbors when each ranks among the ``k1`` nearest neighbors of the
    ...
    false match's own neighbors. The query and every candidate are encoded
    into a k-reciprocal feature, a vector over the candidate set that holds a
    Gaussian weight for each k-reciprocal neighbor and zero elsewhere, and the
    Jaccard distance between the query's feature and a candidate's says how
    much their neighborhoods agree. The final distance the candidates are
