...
class InMemoryImageEmbeddingStore:
    ...
    Each image path is read and embedded with ``embedder``; the resulting vectors
    ...
    def load_from_disk(cls, path: str | pathlib.Path) -> InMemoryImageEmbeddingStore:
        ...
        The embedder is reconstructed and the index is re-trained from the saved
        embeddings using the saved parameters; no image is re-embedded.
        ...
        embedder = embedder_from_dict(state["embedder"])
...
def _embed_single_path(path: str, embedder: Embedder) -> Float32NumpyArray:
    ...
    :param path: Filesystem path of the image to embed.
    :param embedder: Embedder turning the image into a feature vector.
